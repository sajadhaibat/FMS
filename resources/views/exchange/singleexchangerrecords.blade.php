                        ...
                        <div class="card m-b-20">
                            @foreach ($payments as $key=>$payment)
                                @if ($key>0)
                                    @break
                                @endif
                                <div class="card-body"><h4 class="mt-0 header-title">{{ucfirst($payment->exchange->exchanger)}} Records</h4>
                                    @endforeach

                                    <p class="text-muted m-b-30 font-14">See all @foreach ($payments as $key=>$payment)
                                            @if ($key>0)
                                                @break
                                            @endif
                                                {{ucfirst($payment->exchange->exchanger)}}
                                        ...
                                        <div class="table-responsive table-bordered b-0" data-pattern="priority-columns">
                                            <table id="datatable" class="table table-striped">
                                                ...
                                                <tr>
                                                    ...
                                                    <th data-priority="1">Ex-Kaldar amount
                                                    </th>
                                                    <th data-priority="3">Ex-Afghani amount
                                                    </th>
                                                ...
                                                @foreach ($payments as $payment)
                                                    ...
                                                    <tr>
                                                        <td>{{ ucfirst($payment->exchange->exchanger)}}</td>
                                                        <td>{{ $payment->ex_paid_amount }} ({{$payment->rate}}) Kal</td>
                                                        <td>{{ $payment->afghani }} AF</td>
                                                        <td>{{ $payment->date }}</td>
                                                        <td>{{ $payment->persiandate }}</td>
                                                        <td>
                                                            <form action="{{route('exchangerpayment.destroy',$payment->id)}}" method="POST" >
